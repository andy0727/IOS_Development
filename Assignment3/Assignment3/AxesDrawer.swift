...
class AxesDrawer{
    ...
    var minPointPerHashMark: CGFloat = 40
    ...
    func drawAxesInRect(bounds: CGRect, origin: CGPoint, pointsPerUnit: CGFloat)
    {
        ...
        path.stroke()
        drawHashMarkInRect(bounds, origin: origin, pointsPerUnit: abs(pointsPerUnit))
        CGContextRestoreGState(UIGraphicsGetCurrentContext())
    }
    
    
    
    private func drawHashMarkInRect(bounds: CGRect, origin: CGPoint, pointsPerUnit: CGFloat)
    {
        if (origin.x >= bounds.minX && origin.x <= bounds.maxX) || (origin.y >= bounds.minY && origin.y <= bounds.maxY){
            ...
            var unitsPerHashmark = minPointPerHashMark / pointsPerUnit
            ...
            if !CGRectContainsPoint(bounds, origin){
                ...
                startingHashmarkRadius = min(min(leftx, rightx), min(upy, downy)) / pointsPerHashmark + 1
            }
            
            // now create a bounding box inside whose edges those four hashmarks lie
            let boxSize = pointsPerHashmark * startingHashmarkRadius * 2
            var box = CGRect(center: origin, size: CGSize(width: boxSize, height: boxSize))
            ...
            while(!CGRectContainsRect(box, bounds))
            {
                let label = formatter.stringFromNumber((origin.x - box.minX) / pointsPerUnit)!
                if let leftHashmarkPoint = alignedPoint(x: box.minX, y: origin.y , insideBounds: bounds){
                    drawHashmarkAtLocation(leftHashmarkPoint, .Top("-\(label)"))
                }
                if let rightHashmarkPoint = alignedPoint(x: box.maxX, y: origin.y, insideBounds:bounds) {
                    drawHashmarkAtLocation(rightHashmarkPoint, .Top(label))
                }
                if let topHashmarkPoint = alignedPoint(x: origin.x, y: box.minY, insideBounds:bounds) {
                    drawHashmarkAtLocation(topHashmarkPoint, .Left(label))
                }
                if let bottomHashmarkPoint = alignedPoint(x: origin.x, y: box.maxY, insideBounds:bounds) {
                    drawHashmarkAtLocation(bottomHashmarkPoint, .Left("-\(label)"))
                }
                box.inset(dx: -pointsPerHashmark, dy: -pointsPerHashmark)
            }
